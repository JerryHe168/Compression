...
bool Unzipper::extractFile(const std::string& entryName, const std::string& outputPath)
{
    ...
    if (compressionMethod == ZipMethods::STORED) {
        ...
        if (!Utils::ZlibUtils::decompress(compressedData, decompressedData, entry.uncompressedSize)) {
    ...
    uint32_t calculatedCrc = Utils::ZlibUtils::calculateCRC32(decompressedData);
